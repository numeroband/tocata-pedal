...
The controller navigates by **position within the active setlist**, not by absolute program id: `loadPosition`/`movePosition` replace the old hardcoded `% 99`, and the two-digit display shows the position. Incoming Program Change carries an absolute program id but is ignored unless `Setlist::find()` locates it in the active setlist; CC 32 selects a setlist (0 = All, 1..26 stored) and jumps to its first program, ignoring unknown, empty or already-active values.
