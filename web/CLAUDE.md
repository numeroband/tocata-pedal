...
- **`--program-mode-switch <0-7>`** replaces one footswitch with `mode: 'program'` (a pure program-change trigger — no `onActions`/`offActions`/color) instead of a scene switch. Whatever QC scene occupied that position is dropped (a warning is logged per-preset if it had a non-empty label).
