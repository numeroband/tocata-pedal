export default class UF2 {
  ...
  getBlock(idx) {
    if (idx >= this.numBlocks) {
      throw Error(`${idx}: Invalid block index`);
    }
    ...
    if (firstMagic != 0x0A324655) {
      throw Error(`${idx}: Invalid block first magic`);
    }
    const secondMagic = view.getUint32(4, true);
    if (secondMagic != 0x9E5D5157) {
      throw Error(`${idx}: Invalid block second magic`);
    }
    const flags = view.getUint32(8, true);
    if (flags != 0x2000) {
      throw Error(`${idx}: Invalid block flags`);
    }
    const payloadSize = view.getUint32(16, true);
    if (payloadSize > 476) {
      throw Error(`${idx}: Invalid block payload size`);
    }
    const familyId = view.getUint32(28, true);
    if (familyId != 0xe48bff56) {
      throw Error(`${idx}: Invalid block family id`);
    }
    const finalMagic = view.getUint32(508, true);
    if (finalMagic != 0x0AB16F30) {
      throw Error(`${idx}: Invalid block final magic`);
    }
    return {
